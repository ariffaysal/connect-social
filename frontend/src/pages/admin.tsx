  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (newUser.departmentId !== '') {
        if (newUser.departmentId === 'all') {
          payload.allDepartmentsAccess = true;
        } else {
          payload.departmentId = Number(newUser.departmentId);
          payload.allDepartmentsAccess = false;
        }
      }
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {users.map((u) => (
                      <tr key={u.userId}>
                        ...
                        <td className="px-2 py-3">
                          <select
                            value={u.allDepartmentsAccess ? 'all' : (u.departmentId ?? '')}
                            onChange={(e) => {
